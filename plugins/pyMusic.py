...
class pyMusicClass:
    def __init__(self):
        ...
        self.binFolder = os.path.join(self.BASE_DIR, 'pyMusicBIN')
        self.musicFolder = os.path.join(self.binFolder, 'music')
        self.musicFiles = {
        	"rick": os.path.join(self.musicFolder, 'rick.wav'),
        	"crab": os.path.join(self.musicFolder, 'crab.wav'),
        	"default": os.path.join(self.musicFolder, 'default.wav'),
        	"sans": os.path.join(self.musicFolder, 'sans.wav')
        }

    def run(self, arguments):
        args = arguments.split(" ")
        if(args[0] >= "100"):
            Sound.volume_set(int(args[0]))
        music = self.musicFiles[args[1]]
        print(str(music))
        if(args[2] == "yes"):
